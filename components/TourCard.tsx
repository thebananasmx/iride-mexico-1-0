...
import { handleNavigate, slugify } from '../utils';

const TourCard: React.FC<{ tour: Tour }> = ({ tour }) => {
  const tourUrl = `/tours/${slugify(tour.title)}`;

  return (
    <a 
      href={tourUrl}
      onClick={(e) => handleNavigate(e, tourUrl)}
      className="group bg-white rounded-2xl overflow-hidden shadow-lg transition-all duration-200 flex flex-col h-full border border-gray-100 transform hover:-translate-y-2"
    >
